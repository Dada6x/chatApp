...
exports.sendPrivateMessage = async (req, res) => {
  try {
    const { text, imageBase64, receiver_id } = req.body;
    ...
    // 🔥 Emit to specific users (sender and receiver)
    const io = getIO();
    // Emit to receiver's room
    io.to(`user:${receiver_id}`).emit("private:new-message", message);
    // Also emit to sender's room (for multi-device support)
    io.to(`user:${userId}`).emit("private:new-message", message);
