...
export const sendPrivateMessage = createAsyncThunk(
  ...
  async ({ text, recipientId, imageBase64 }, { getState, rejectWithValue, dispatch }) => {
    try {
      const { auth } = getState();
      const response = await axios.post(ENDPOINTS.PRIVATE_MESSAGES, { text, imageBase64, receiver_id: recipientId }, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
      const message = socketService.mapPrivateMessage(response.data);
      dispatch(addPrivateMessage(message));
      return { text, recipientId, imageBase64 };
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
  }
...
export const sendMessage = createAsyncThunk(
  'chat/sendMessage',
  async ({ text, imageBase64 }, { getState, rejectWithValue, dispatch }) => {
    try {
      const { auth } = getState();
      const response = await axios.post(ENDPOINTS.HALL_MESSAGES, { text, imageBase64 }, {
        headers: { Authorization: `Bearer ${auth.token}` },
      });
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    addPrivateMessage: (state, action) => {
      ...
      console.log('Adding private message:', action.payload, 'chatKey:', chatKey);
      if (!state.privateMessages[chatKey]) {
        state.privateMessages[chatKey] = [];
      }
      if (!state.privateMessages[chatKey].find(m => m.id === action.payload.id)) {
        state.privateMessages[chatKey].push(action.payload);
        console.log('Added message to privateMessages[chatKey]');
      } else {
